...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  const isOpenRouter = finalApiUrl.includes("openrouter.ai");

  // Adjust payload for OpenRouter compatibility
  const requestPayload = { ...payload };
  if (isOpenRouter && requestPayload.thinking) {
  ...
  if (isOpenRouter) {
    headers["HTTP-Referer"] = process.env.OPENROUTER_REFERER || "https://smartagent.local";
    headers["X-Title"] = "SmartAgent";
  }
