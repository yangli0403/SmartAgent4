...
const CHITCHAT_PATTERNS: RegExp[] = [
  ...
  // 导航类指令
  /^(帮我)?导航.*/i,
  /^(帮我)?(去|到|怎么去|路线|规划路线|导航到|带我去).*/i,
  /^(查一下|查查|搜一下).{0,10}(天气|路线|地图|附近).*/i,
  // 音乐类指令
  /^(播放|放|听|来一首|搜索).{0,20}(歌|音乐|歌曲)/i,
  /^(搜索|搜|找).{0,10}(歌|歌手|歌单|专辑)/i,
  // 文件类指令
  /^(打开|找|搜索|查找).{0,10}(文件|文档|照片|图片|视频)/i,
  /^(分析|查看|扫描).{0,10}(磁盘|空间|[cCdDeE]盘|目录)/i,
  // 办公类指令
  /^(发|发送|帮我发).{0,10}(飞书|消息)/i,
  /^(创建|建|帮我建).{0,10}(日程|群|群组)/i,
  // 新闻类指令
  /^(查|看|帮我查|推送).{0,10}(新闻|资讯|热点|头条)/i,
  // 通用知识问答（无需个人记忆）
  /^(什么是|解释一下|介绍一下|告诉我).{0,30}$/i,
  // ================= 车控/设备控制指令（无需个人偏好记忆） =================
  // 空调控制
  /^(帮我)?(打开|关闭|关掉|开启|调到|设置|调低|调高|降低|升高).{0,10}(空调|车内温度|温度|风量|风速|制冷|制热|暖风|冷风)/i,
  /^空调.{0,20}(打开|关闭|关掉|开启|调到|设置|调低|调高|\d+度)/i,
  /^(车内)?温度.{0,10}(调到|设置|调低|调高|降到|升到).{0,10}\d+/i,
  // 灯光控制
  /^(帮我)?(打开|关闭|关掉|开启|调亮|调暗|开|关).{0,10}(大灯|车灯|内饰灯|氛围灯|远光|近光|双闪|转向灯)/i,
  /^(大灯|车灯|内饰灯|氛围灯).{0,10}(打开|关闭|关掉|开启|调亮|调暗)/i,
  // 车窗/天窗控制
  /^(帮我)?(打开|关闭|关掉|开启|升起|降下|摇上|摇下).{0,10}(车窗|天窗|玻璃)/i,
  // 座椅控制
  /^(帮我)?(调整|调节|前移|后移|升高|降低|加热|通风).{0,10}(座椅|座位|靠背|坐姿)/i,
  // 白噪音/音效控制
  /^(帮我)?(播放|打开|关闭|关掉|开启|停止).{0,10}(白噪音|雨声|海浪声|自然音效|环境音)/i,
  // 组合车控指令（多个控制项用逗号/顿号分隔）
...
const MEMORY_RELATED_PATTERNS: RegExp[] = [
  ...
  /你(了解|知道)我/,
  // 场景/模式触发（需要检索用户自定义场景）
  /(开启|启动|激活|关闭|退出|结束).{0,10}(模式|场景|设置)/,
  /(午睡|睡觉|睡眠|起床|上班|下班|回家|出门|运动|工作|学习).{0,5}(模式|场景)/,
  /我要(睡觉|休息|起床|出发|上班|下班|回家|运动|学习)/,
  /帮我(开启|启动|激活|设置).{0,15}(模式|场景)/,
];
...
- 用户询问与自身相关的问题
- 用户触发场景/模式（如"开启午睡模式"、"我要睡觉了"、"开始工作模式"）
- 用户要求执行自定义操作序列（需要查找用户保存的场景配置）
