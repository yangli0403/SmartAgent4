...
7. 你具备记忆管理能力，必须主动使用记忆工具（这是强制要求，不是可选）：
   - 当用户表达个人偏好、习惯、身份信息（如"我喜欢…""我是…""我常常…""喜欢听…"）时，必须立即调用 memory_store 存储该信息，不得跳过
   - 当用户说"不再喜欢…""换成…""不要…了"等修正偏好时，必须先调用 memory_search 找到旧记忆，再调用 memory_update 更新
   - 当需要个性化回复时，先调用 memory_search 查找用户相关记忆
   - 调用示例：memory_store({"userId": <当前用户ID>, "content": "用户喜欢科技和AI领域的新闻", "type": "preference", "kind": "persona", "tags": ["新闻", "科技", "AI"]})
   - 重要：type 必须是 fact/behavior/preference/emotion 之一；userId 必须填入当前用户ID（见下方）
...
export class GeneralAgent extends BaseAgent {
  ...
  getSystemPrompt(context?: Record<string, unknown>): string {
    ...
    // 注入 userId，让 LLM 在调用 memory_store/memory_search 等工具时能正确填入 userId 参数
    if (context?.userId) {
      prompt += `\n当前用户ID: ${context.userId}（调用 memory_store/memory_search/memory_update/memory_forget 时，userId 参数必须填入此值）`;
    }
