...
**智能对话交互系统** — 整合个性引擎、记忆系统（含**向量语义检索** + **智能预检索决策** + **质量门控**）、情感表达、**AIRI 前端角色舞台（Live2D）**与**多智能体协同架构**的多模态 AI 助手平台。
...
- **AIRI 前端角色舞台**：基于 PixiJS + pixi-live2d-display 的前端渲染层，支持表情驱动、动作播放、口型联动和闲置管理
...
| 测试 | Vitest 2 + @vitest/coverage-v8 + jsdom |
| 包管理 | pnpm 10 |
...
│   └── src/
│       ├── components/
│       │   ├── airi-stage/              # 🆕 AIRI 前端角色舞台
│       │   │   ├── AiriStageContainer.tsx # Live2D 画布容器
│       │   │   └── BridgeStatusPanel.tsx  # Bridge 状态面板
│       │   └── cockpit/                 # 驾驶舱组件
│       ├── hooks/
│       │   ├── useExpressionDriver.ts   # 🆕 表情驱动器
│       │   ├── useMotionDriver.ts       # 🆕 动作驱动器
│       │   ├── useLipsyncDriver.ts      # 🆕 口型驱动器
│       │   └── useIdleManager.ts        # 🆕 闲置管理器
│       ├── pages/                       # 页面（Chat, Cockpit, Memories, Settings）
│       └── lib/
│           ├── airi-stage/              # 🆕 AIRI 舞台核心库
│           │   ├── types.ts             # 舞台事件类型定义
│           │   ├── stageEventBus.ts     # 事件总线
│           │   ├── useStageStore.ts     # Zustand 状态管理
│           │   ├── expressionMapping.ts # 表情映射配置
│           │   └── motionMapping.ts     # 动作映射配置
│           └── ...                      # tRPC client 等
├── server/                              # Node.js 后端
│   ├── agent/                           # Agent 架构
...
- **后端测试**：654 个（651 通过，3 个需数据库）
- **前端测试**：71 个（AIRI 舞台模块，全部通过）
- **AIRI 核心库覆盖率**：语句/分支/函数 100%
- **全量测试文档**：后端 [TESTING.md](./TESTING.md) / 前端 [docs/TESTING_AIRI_STAGE.md](./docs/TESTING_AIRI_STAGE.md)
...
| [docs/PRODUCT_SPEC_AIRI_STAGE.md](./docs/PRODUCT_SPEC_AIRI_STAGE.md) | 第八轮 AIRI 舞台产品规格 |
| [docs/ARCHITECTURE_AIRI_STAGE.md](./docs/ARCHITECTURE_AIRI_STAGE.md) | 第八轮 AIRI 舞台架构设计 |
| [docs/INTERFACE_DESIGN_AIRI_STAGE.md](./docs/INTERFACE_DESIGN_AIRI_STAGE.md) | 第八轮 AIRI 舞台接口设计 |
| [docs/TESTING_AIRI_STAGE.md](./docs/TESTING_AIRI_STAGE.md) | 第八轮 AIRI 舞台测试文档 |
...
- [x] **第八轮迭代：AIRI 前端角色舞台集成（Live2D + 事件驱动 + 状态机）**
