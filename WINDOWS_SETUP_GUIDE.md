...
| **PostgreSQL** | 15+ | 方式一：[PostgreSQL Installer](https://www.postgresql.org/download/windows/)；方式二：Docker Desktop | 数据库 |
| **Git** | 2.30+ | [git-scm.com](https://git-scm.com/) | 代码管理 |

如果您选择使用 Docker 来运行 PostgreSQL（推荐），还需要安装 [Docker Desktop for Windows](https://www.docker.com/products/docker-desktop/)。
...
这将在本地 5432 端口启动一个 PostgreSQL 16 实例，数据库名为 `smart_agent`，用户名 `postgres`，密码为 `password`。

**方式二：使用本地安装的 PostgreSQL**

如果您已安装 PostgreSQL，请手动创建数据库：
...
```env
# 数据库连接（根据您的实际情况修改）
DATABASE_URL=postgresql://postgres:password@localhost:5432/smart_agent
...
A: 请检查 PostgreSQL 是否已启动，以及 `.env` 中的 `DATABASE_URL` 是否正确。如果使用 Docker，运行 `docker ps` 确认容器状态。
...
A: 如果您之前使用的是 MySQL 版本，请参考以下步骤：
1. 导出 MySQL 数据（`mysqldump --compatible=postgresql smart_agent > dump.sql`）
2. 更新 `.env` 中的 `DATABASE_URL` 为 PostgreSQL 连接字符串
3. 运行 `docker-compose down -v` 清除旧 MySQL 容器
4. 运行 `docker-compose up -d` 启动新 PostgreSQL 容器
5. 运行 `pnpm run db:push` 创建新表结构
6. 使用 `pgloader` 或手动导入数据
